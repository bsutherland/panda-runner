...
{
        /**
        *   Creates a string with variable substitutions. Very similiar to printf, especially python's printf
        ...
        */
         public function printf(raw : String, ...rest) : String{
			...
			*   .xx -> [optional] the precision with which numbers will be formatted  
...
function padString(str:String, paddingNum:int, paddingChar:String=" ") : String
{
	...
	var n:uint = paddingNum < 0 ? -paddingNum : paddingNum;
	var pad:String = new Array(n).join(paddingChar).substr(0, n - str.length);
    return paddingNum < 0 ? str.concat(pad) : pad.concat(str);
}